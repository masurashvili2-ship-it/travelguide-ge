...
function uploadDir(): string {
	const override = process.env.WTD_UPLOAD_DIR?.trim();
	if (override) return path.resolve(override);
	const prodClient = path.join(process.cwd(), 'dist', 'client', 'uploads', 'what-to-do');
	if (process.env.NODE_ENV === 'production' && existsSync(path.join(process.cwd(), 'dist', 'client'))) {
		return prodClient;
	}
	return path.join(process.cwd(), 'public', 'uploads', 'what-to-do');
}
...
export const POST: APIRoute = async ({ request, locals }) => {
	...
	if (!base) {
		return new Response(
			JSON.stringify({
				error:
					'Cannot derive a Latin file name from the title. Fill in the URL slug (e.g. tbilisi-night), then upload.',
			}),
			{ status: 400, headers: { 'Content-Type': 'application/json' } },
		);
	}
	...
	const url = `/uploads/what-to-do/${name}`;
