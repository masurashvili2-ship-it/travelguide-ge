...
export const POST: APIRoute = async ({ request, locals }) => {
	...
	const added = addContentSubmission('tours', payload, { userId: user.id, email: user.email });
	if (!added.ok) {
	...
	notifyAdmin(
		'[TravelGuide] New tour submission',
		`<p>A new <strong>tour</strong> submission has arrived from <strong>${user.email}</strong>.</p>
<p>Submission ID: <code>${added.id}</code></p>
<p>Review it in the <a href="/en/admin/tours">admin panel</a>.</p>`,
	);
