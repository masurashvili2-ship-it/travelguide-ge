...
export const PATCH: APIRoute = async ({ request, params, locals }) => {
	...
	if (!isValidGuideId(guideId) || !isValidGuideId(commentId) || !getGuideById(guideId)) {
	...
	const result = await updateTourComment('guides', guideId, commentId, user, body, rating);
	if (!result.ok) {
...
export const DELETE: APIRoute = async ({ params, locals, request }) => {
	...
	const result = await deleteTourComment('guides', guideId, commentId, user);
	if (!result.ok) {
		const status = result.error === 'Forbidden' ? 403 : result.error === 'Comment not found' ? 404 : 400;
		return new Response(JSON.stringify({ error: result.error }), {
			status,
