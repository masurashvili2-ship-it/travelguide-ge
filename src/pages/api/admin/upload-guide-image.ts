...
export const POST: APIRoute = async ({ request, locals }) => {
	...
	const titleField = firstNonEmptyField('tour_title') || firstNonEmptyField('title');
	...
	if (!titleField && !slugField) {
		return new Response(
			JSON.stringify({ error: 'Enter the guide name or URL slug before uploading.' }),
	...
	const dir = resolveUploadDir('guides');
	mkdirSync(dir, { recursive: true });
	let base = slugifyTitleForUploadFilename(titleField);
	if (!base) base = slugifyTitleForUploadFilename(slugField);
	if (!base) {
		return new Response(
			JSON.stringify({ error: 'Cannot derive a file name from the guide name. Fill in the URL slug first.' }),
			{ status: 400, headers: { 'Content-Type': 'application/json' } },
		);
	}
