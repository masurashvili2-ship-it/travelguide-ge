...
export const POST: APIRoute = async ({ request, locals }) => {
	...
	const titleField =
		firstNonEmptyField('region_title') ||
		firstNonEmptyField('tour_title') ||
		firstNonEmptyField('title');
	const slugField =
		firstNonEmptyField('region_slug') || firstNonEmptyField('tour_slug') || firstNonEmptyField('slug');
	...
	if (!titleField && !slugField) {
		return new Response(
			JSON.stringify({ error: 'Enter the place title or URL slug before uploading (needed for file names).' }),
			{
				status: 400,
				headers: { 'Content-Type': 'application/json' },
			},
	...
	const dir = resolveUploadDir('regions');
	...
	if (!base) {
		return new Response(
			JSON.stringify({
				error:
					'Cannot derive a Latin file name from the title. Fill in the URL slug (e.g. ajara), then upload.',
			}),
			{
				status: 400,
				headers: { 'Content-Type': 'application/json' },
	...
	const url = `/uploads/regions/${name}`;
